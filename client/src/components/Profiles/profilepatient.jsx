  ...
  useEffect(() => {
    if (patientID) {
      const fetchProfile = async () => {
        try {
          console.log('Fetching patient profile with ID:', patientID);
  ...
  const handleUpdateClick = async () => {
    if (isEditing) {
        ...
        try {
          ...
          const response = await updatePatientbyId(patientID, updatedProfile);
          console.log('Profile updated successfully:', response);
          
          // Update local state with new profile data
          setProfile(updatedProfile);
          console.log('Updatingile:', profile);
          // Exit edit mode
          setIsEditing(false);
      ...
      <div className="mb-4">
        ...
        {isEditing ? (
          <input
            ...
            name="DateNaissance"
            format = 'yyyy/MM/dd'
            value={updatedProfile.DateNaissance?format(new Date(updatedProfile.DateNaissance ), 'yyyy/MM/dd'):  ""}
            onChange={handleChange}
            className="w-full border-gray-300 rounded-lg p-2 mt-1"
          />
        ) : (
